...
def test_partition_leaves_by_durations_02( ):
   v = Voice([divide([1, -4, 1], (1, 4)), divide([1, -4, 1, 1], (1, 4))])
   assert len(v.leaves) == 7
   result = partitionLeavesByDurations(v.leaves, [(1, 2)])
   assert len(result) == 1
...
def test_partition_leaves_by_durations_03( ):
   ...
   result = partitionLeavesByDurations(v.leaves, [(1, 32), (3, 32)])
   assert len(result) == 4
...
def test_partition_leaves_by_durations_04( ):
   v = Voice(Note(0, (1, 32)) * 8)
   assert len(v.leaves) == 8
   result = partitionLeavesByDurations(v.leaves, [(3, 32)])
   assert len(result) == 3
