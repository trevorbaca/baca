...
class ScoreSpecification(Specification):
    ...
    def add_divisions_to_voice(self, voice):
        ...
        self.add_segment_division_list_to_segment_payload_context_dictionarys_for_voice(voice, segment_division_lists)
    ...
    def add_segment_division_list_to_segment_payload_context_dictionarys_for_voice(self, voice, segment_division_lists):
        ...
        for segment, segment_division_list in zip(self.segments, segment_division_lists):
            ...
            segment.payload_context_dictionary[voice.name]['segment_pairs'] = [x.pair for x in segment_division_list]
    ...
    def change_segment_division_tokens_to_region_division_tokens(self, segment_division_tokens):
        ...
        for segment_division_token in segment_division_tokens:
            if segment_division_token.fresh or segment_division_token.truncate:
                #value, duration = segment_division_token.value, segment_division_token.duration
                #region_division_token = RegionDivisionToken(value, duration)
    ...
    def interpret_segment_divisions(self):
        for segment in self.segments:
            ...
            if not settings:
                settings = []
                existing_settings = self.resolved_settings_context_dictionary.get_settings(attribute_name='divisions')
    ...
    def interpret_segment_rhythms(self):
        for segment in self.segments:
            ...
            if not settings:
                settings = []
                existing_settings = self.resolved_settings_context_dictionary.get_settings(attribute_name='rhythm')
                for existing_setting in existing_settings:
                    setting = copy.deepcopy(existing_setting)
                    setting.segment_name = segment.name
                    setting.fresh = False
    ...
    def store_setting(self, setting):
        ...
        segment.resolved_settings_context_dictionary[context_name][resolved_setting.attribute_name] = resolved_setting
