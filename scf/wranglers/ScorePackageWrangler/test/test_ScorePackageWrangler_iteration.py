...
def test_ScorePackageWrangler_iteration_04():
    ...
    assert 'archipel' in wrangler.list_asset_short_names()
    assert 'archipel' not in wrangler.list_asset_short_names(head='aracilik')
    assert wrangler.list_asset_short_names(head='asdf') == []
