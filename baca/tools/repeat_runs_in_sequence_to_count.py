...
def repeat_runs_in_sequence_to_count(sequence, tokens):
    ...
    as the total number of occurrences of ``sequence[start:start+length]`` to
    appear in `sequence` after execution, but rather as the number of new
    occurrences of ``sequence[start:start+length]`` to appear in `sequence`
    after execution.

    Wraps newly created subruns in tuples.  That is, returns output with one
    more level of nesting than given in input.

    Inserts ``10`` count of ``sequence[:2]`` at ``sequence[2:2]``:
    ...
    Inserts ``5`` count of ``sequence[10:12]`` at ``sequence[12:12]`` and
    then inserts ``5`` count of ``sequence[:2]`` at ``sequence[2:2]``:
    ...
    Wraps around the end of `sequence` whenever ``len(sequence) < start +
    length``.

    Inserts ``2`` count of ``[18, 19, 0, 1]`` at ``sequence[2:2]``:
    ...
    Inserts ``2`` count of ``[18, 19, 0, 1, 2, 3, 4]`` at ``sequence[4:4]``:
