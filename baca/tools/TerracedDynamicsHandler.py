...
class TerracedDynamicsHandler(Handler):
    ...
    def __init__(
        ...
        if dynamics is not None:
            for dynamic in dynamics:
                if not abjad.indicatortools.Dynamic.is_dynamic_name(dynamic):
    ...
    def __call__(self, logical_ties, offset=0):
        ...
        Returns none.
        '''
        dynamics = abjad.datastructuretools.CyclicTuple(self.dynamics)
        for i, logical_tie in enumerate(logical_ties):
            ...
            if i == 0 and self.always_first_note:
                ...
                if written_duration < self.minimum_duration:
                    continue
            #abjad.indicatortools.Dynamic(dynamic_name)(note_or_chord)
    ...
    def always_first_note(self):
        r'''Is true when handler always affixes dynamic to first note,
