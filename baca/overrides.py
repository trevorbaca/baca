...
class OverrideCommand(scoping.Command):
    ...
    def _call(self, argument=None) -> None:
        ...
        string = abjad.LilyPondGrobNameManager.make_lilypond_override_string(
        ...
        if once:
            return
        string = abjad.LilyPondGrobNameManager.make_lilypond_revert_string(
...
def time_signature_extra_offset(
    pair: abjad.NumberPair,
    selector: abjad.Expression = classes.Expression().select().hleaf(0),
) -> OverrideCommand:
    r"""
...
def time_signature_stencil_false(
    selector: abjad.Expression = classes.Expression().select().hleaves(),
) -> OverrideCommand:
    """
