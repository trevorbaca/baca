...
#   assert all([isinstance(x, (int, float, long, Fraction)) for x in s])
#
#   result = [ ]
#
#   if cycle == False:
...
#   return result
   ...
   if cycle == False:
      return listtools.partition_by_weights_not_less_than(l, s, cyclic = False, overhang = False)
   elif cycle == 'knife':
      return listtools.partition_by_weights_not_less_than(l, s, cyclic = True, overhang = False)
   elif cycle == 'sheet':
