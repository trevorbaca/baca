...
def _trim_matching_chord(logical_tie, pitch_object):
    if isinstance(logical_tie.get_head(), abjad.Note):
        return
    assert isinstance(logical_tie.get_head(), abjad.Chord), repr(logical_tie)
...
def imbricate(
    ...
    *,
    ...
    for logical_tie, original_logical_tie in pairs:
        if isinstance(logical_tie.get_head(), abjad.Rest):
            for leaf in logical_tie:
                duration = leaf.written_duration
                skip = abjad.Skip(duration)
                abjad.mutate.replace(leaf, [skip])
        elif isinstance(logical_tie.get_head(), abjad.Skip):
            pass
        elif _matches_pitch(logical_tie.get_head(), pitch_number):
