    ...
    for wrapper in abjad.get.wrappers(leaf):
        if isinstance(wrapper.unbundle_indicator(), abjad.LilyPondLiteral):
            if wrapper.unbundle_indicator().argument == "":
                continue
        if tag.string not in wrapper.get_tag().string:
    ...
    for leaf in abjad.iterate.leaves(score, pitched=True):
        ...
        for note_head in note_heads:
            ...
            if staff_position.number == 0:
                ...
                bundle = abjad.bundle(
                    wrapper.get_indicator(), r"- \tweak direction #up"
                )
                abjad.attach(bundle, leaf, tag=wrapper.get_tag())
                break
...
def _collect_persistent_indicators(
    ...
    for name, dependent_wrappers in name_to_wrappers.items():
        ...
        for wrapper in wrappers:
            ...
            editions = wrapper.get_tag().editions()
            if editions:
    ...
    *,
    ...
    for memento in mementos:
        ...
        if isinstance(result.previous_indicator, abjad.TimeSignature):
            ...
            tag_ = wrapper.get_tag().append(result.edition)
            wrapper.set_tag(tag_)
    ...
    for leaf in abjad.iterate.leaves(score):
        for wrapper in abjad.get.wrappers(leaf):
            ...
            if 0 <= synthetic_offset:
                continue
            if "REAPPLIED" in wrapper.get_tag().string:
                string = wrapper.get_tag().string
...
def _shift_measure_initial_clefs(
    ...
    for staff in abjad.iterate.components(score, abjad.Staff):
        for leaf in abjad.iterate.leaves(staff):
            ...
            if wrapper is None or not wrapper.get_tag():
                continue
            if _tags.EXPLICIT_CLEF.string not in wrapper.get_tag().words():
                continue
            measure_number = offset_to_measure_number.get(start_offset)
            if measure_number is None:
                continue
    ...
    for leaf in abjad.iterate.leaves(score):
        ...
        for wrapper in wrappers:
            if wrapper.get_tag() is None:
                continue
            for tag in tags:
                if tag.string in wrapper.get_tag().words():
    ...
    *,
    ...
    for i, skip in enumerate(skips):
        ...
        if wrapper is not None:
            ...
            metronome_mark_tweaks = getattr(wrapper.get_indicator(), "tweaks", ())
    ...
    for leaf in abjad.iterate.leaves(score):
        for wrapper in abjad.get.wrappers(leaf):
            ...
            if not getattr(indicator, "persistent", False):
                continue
            if wrapper.get_tag() is not None and has_persistence_tag(wrapper.get_tag()):
                continue
